...
public class PolygonActivity extends Activity implements
	...
	private void setUpMap() {
		...
		aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(MapConstant.BEIJING, 5));// 设置指定的可视区域地图
		// 绘制一个长方形
		aMap.addPolygon(new PolygonOptions()
				.addAll(createRectangle(MapConstant.SHANGHAI, 1, 1))
		...
		for (int i = 0; i <= numPoints; i++) {
			options.add(new LatLng(MapConstant.BEIJING.latitude
					+ semiVerticalAxis * Math.sin(i * phase),
					MapConstant.BEIJING.longitude + semiHorizontalAxis
							* Math.cos(i * phase)));
		}
		// 绘制一个椭圆
