package rxandroid.com.view.map.overlay.activity;
...
import rxandroid.com.utils.MapConstant;
import rxandroid.com.utils.ToastUtil;
...
public class CustomMarkerActivity extends Activity implements OnMarkerClickListener,
	...
	private void addMarkersToMap() {
		...
		TextOptions textOptions = new TextOptions().position(MapConstant.BEIJING)
		...
		aMap.addMarker(new MarkerOptions().anchor(0.5f, 0.5f)
				.position(MapConstant.CHENGDU).title("成都市")
				.snippet("成都市:30.679879, 104.064855").draggable(true));

		markerOption = new MarkerOptions();
		markerOption.position(MapConstant.XIAN);
		...
		aMap.addMarker(new MarkerOptions().anchor(0.5f, 0.5f)
				.position(MapConstant.ZHENGZHOU).title("郑州市").icons(giflist)
	...
	public void jumpPoint(final Marker marker) {
		...
		Point startPoint = proj.toScreenLocation(MapConstant.XIAN);
		...
		handler.post(new Runnable() {
			...
			public void run() {
				...
				float t = interpolator.getInterpolation((float) elapsed
						/ duration);
				double lng = t * MapConstant.XIAN.longitude + (1 - t)
						* startLatLng.longitude;
				double lat = t * MapConstant.XIAN.latitude + (1 - t)
	...
	public void onMapLoaded() {
		...
		LatLngBounds bounds = new LatLngBounds.Builder()
				.include(MapConstant.XIAN).include(MapConstant.CHENGDU)
				.include(latlng).include(MapConstant.ZHENGZHOU).include(MapConstant.BEIJING).build();
